---
layout: course
title: Program-Based Circuit Theory I (DC)
subtitle: 프로그램 기반 회로 이론 I (DC)
description: 0000127046 • 2025년 1학기 • 전북대학교
logo: jbnu-logo-2.png
img: assets/img/books/electric-circuits.jpg
importance: 2
category: 2025-1
data_file: 2025/jbnu_circuits_lectures

grading:
  attendance: 20
  midterm: 30
  final: 30
  homework: 20

information:
  - section: 0000127046
    time: 월 1-4pm
    location: 인문대학 2호관 504호
    kakaotalk: https://open.kakao.com/o/gNhLNgjh

Main-Text:
  - text: "주교재"
    author: "Alexander, Charles K., Matthew N. O. Sadiku"
    title: <strong>ISE Fundamentals of Electric Circuits</strong>, 7th Edition
    publisher: "McGraw-Hill Education | 2020년 2월 28일"
    link: "https://www.amazon.com/ISE-Fundamentals-of-Electric-Circuits/dp/1260570797"
    image: books/electric-circuits.jpg

Supplementary:
  - text: "부교재"
    author: "O'Malley, John"
    title: <strong>Schaum's Outline of Basic Circuit Analysis</strong>, 2nd Edition
    publisher: "McGraw Hill | 2011년 02월 17일"
    link: "https://www.amazon.com/Schaums-Outline-Circuit-Analysis-Outlines/dp/0071756434/"
    image: books/schaums-circuits.jpg

  - text: "부교재"
    author: "Flux, Jamie"
    title: "<strong>Linear Algebra in Circuit Design: With Python</strong>"
    publisher: "Independently published | 2024년 10월 02일"
    link: "https://www.amazon.com/Linear-Algebra-Circuit-Design-Engineering/dp/B0DJDF1D63/"
    image: books/linear-alg-circuits.jpg
---

DC 회로 이론의 기본 개념(전압, 전류, 저항, 키르히호프 법칙, 노드/메쉬 분석)을 학습하고 Python으로 회로 해석을 구현합니다.

This course covers DC circuit theory fundamentals — voltage, current, resistance, Kirchhoff's laws, node/mesh analysis — with Python-based circuit analysis implementations.
