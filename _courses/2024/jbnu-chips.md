---
layout: course
title: Semiconductor Processing
subtitle: 반도체공정
description: 0000125132 • 2024년 2학기 • 전북대학교
logo: jbnu-logo-2.png
img: assets/img/books/semiconductor-fabrication.jpg
importance: 3
category: 2024-2
data_file: 2024/jbnu_chips_lectures

grading:
  attendance: 20
  midterm: 30
  final: 30
  homework: 20

information:
  - section: 0000125132
    time: 월 4-6pm & 수 4-5pm
    location: 인문대학 2호관 501호
    kakaotalk: https://open.kakao.com/o/gAu7mtMg

Main-Text:
  - text: "주교재"
    author: "Lian, Yaguang"
    title: "<strong>Semiconductor Microchips and Fabrication:</strong> A Practical Guide to Theory and Manufacturing"
    publisher: "Wiley-IEEE Press | 2023년 06월 15일"
    link: "https://www.yes24.com/Product/Goods/106153977"
    image: books/semiconductor-fabrication.jpg
---

반도체 소자의 제조 공정(산화, 리소그래피, 식각, 증착, 이온 주입 등)의 이론과 실제를 학습합니다.

This course covers semiconductor fabrication processes — oxidation, lithography, etching, deposition, ion implantation — bridging theory to manufacturing practice.
