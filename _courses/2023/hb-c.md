---
layout: course
init: HB
title: Advanced C Programming
subtitle: 고급 C 프로그래밍
description: INFO2104 • 2023년 2학기 • 한밭대학교
logo: https://res.cloudinary.com/dobann7wi/image/upload/q_auto/f_auto/v1777038668/hanbat-logo_mvle6l.png
img: assets/img/books/effective-c.jpg
importance: 4
category: 2023
data_file: 2023/hb_c_lectures

grading:
  attendance: 10
  homework: 30
  project: 10
  midterm: 20
  final: 30

information:
  - section: INFO2104
    time: 금 6789 | Fri 2pm-5:50pm
    location: N4동 316호
    kakaotalk: https://open.kakao.com/o/g7tnYPDf

Main-Text:
  - text: "주교재"
    author: "로버트 C. 시코드 저 / 박정재, 장기식, 장준원 역"
    title: "<strong>Effective C: 전문적인 C 프로그래밍 입문서</strong>"
    publisher: "에이콘출판사 | 2023년 06월 30일"
    link: "https://www.yes24.com/Product/Goods/119423118"
    image: books/effective-c.jpg

Supplementary:
  - text: "부교재"
    author: "주우석 저"
    title: <strong>전공자를 위한 C 언어 프로그래밍</strong>
    publisher: "한빛아카데미 | 2020년 11월 27일"
    link: "https://www.yes24.com/Product/Goods/67476379"

  - text: "부교재"
    author: "서현우 저"
    title: <strong>혼자 공부하는 C 언어 [개정판]</strong>
    publisher: "한빛미디어 | 2023년 05월 18일"
    link: "https://www.yes24.com/Product/Goods/118982111"
---

시간이 지나도 변하지 않는 전문적이고 안전하며 이식 가능한 C 코드 작성법을 학습합니다. 포인터, 동적 메모리 할당, 문자열 처리, I/O, 전처리기, 프로그램 구조, 디버깅 및 테스트까지 C 언어의 고급 기법을 다룹니다.

This course teaches professional, safe, and portable C programming — covering pointers, dynamic memory, strings, I/O, the preprocessor, program structure, and debugging and testing.
